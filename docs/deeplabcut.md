...
Argus Clicker includes a specialized mode for labeling and refining DeepLabCut projects. This feature allows you to use Argus as an alternative to the DeepLabCut GUI for adding or correcting labels, with some advantages including keyboard navigation between labeled frames and the ability to refine machine-generated labels without overwriting existing manual labels.
...
4. Select the associated video file when prompted (Argus will first check the `../videos/` directory)
...
3. **Combines the data intelligently**: 
...
- **Regular Arrow Keys**: Navigate one frame at a time (as in normal Argus mode)
- **Display All Tracks**: Automatically enabled in DLC mode to show all body parts simultaneously

These shortcuts make it easy to quickly navigate between frames that need labeling or review, skipping over unlabeled frames in the video.
...
2. **Add or correct labels**: Click on the video to place or move a marker for the currently selected body part
   - Use the options menu (`O` key) to switch between different body parts (tracks)
   - The current track is highlighted with a larger marker
3. **Navigate between frames**: Use Shift+Arrow keys to jump between labeled frames, or arrow keys to move frame-by-frame
4. **Note**: Auto-advance is disabled in DLC mode to give you more control when clicking points

#### Saving Your Work

When you're ready to save:

1. Press `S` to open the save dialog, or press `O` to open the options menu and click "Specify" to set the save location
2. In the options menu (`O` key), ensure the save format is set to "DeepLabCut .h5"
3. Save your work - Argus will create both a `.h5` and a `.csv` file in DeepLabCut format
